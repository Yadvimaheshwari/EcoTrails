...
import { Discovery, DiscoveryType, CapturedDiscovery } from '@/types/discovery';
...
export async function captureDiscovery(
  ...
  if (USE_MOCK_DISCOVERIES) {
    return mockCaptureDiscovery(hikeId, discoveryId, location, notes);
  }
...
/**
 * Stop hike and get summary
 */
export async function stopHike(
  hikeId: string,
  distance: number,
  durationMinutes: number,
  capturedDiscoveries: CapturedDiscovery[],
  earnedBadges: Badge[]
): Promise<{ success: boolean; summary: HikeSummaryData | null }> {
  try {
    const response = await api.post(`/api/v1/hikes/${hikeId}/stop`, {
    ...
    console.error('[DiscoveryService] Failed to stop hike:', error);
    ...
    return {
      ...
      summary: {
        ...
        distance,
        durationMinutes,
        discoveriesCount: capturedDiscoveries.length,
...
function generateMockDiscoveries(trailId: string): Discovery[] {
  // Seed random based on trailId for consistent results
  const seed = hashString(trailId);
  const random = seededRandom(seed);

  const count = Math.floor(random() * 5) + 3; // 3-7 discoveries per trail
  const discoveries: Discovery[] = [];

  // Sample discovery types with weighted distribution
  ...
  for (let i = 0; i < count; i++) {
    ...
    discoveries.push({
      ...
      trailId,
      lat: 37.7749 + (random() - 0.5) * 0.02, // Mock lat around SF
      lng: -122.4194 + (random() - 0.5) * 0.02, // Mock lng
      type,
...
function mockCaptureDiscovery(
  ...
  // Determine badge type based on discovery ID (extract type from mock ID pattern)
...
/**
 * Extract discovery type from mock ID
 */
function getDiscoveryTypeFromId(discoveryId: string): DiscoveryType {
  // In real impl, this would be from the discovery object
  // For mock, we'll default to a common type
  return 'scenic_view';
}
  ...
  wildlife: [
    {
      ...
      longText: 'White-tailed deer are most active at dawn and dusk. Look for browse marks on nearby vegetation.',
    },
    {
      title: 'Bird Nest',
